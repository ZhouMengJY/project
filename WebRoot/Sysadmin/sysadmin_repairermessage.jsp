...
<div>   	
  <c:choose>
    ...
    <table border="1" >
	  ...
	  <c:forEach var="repairer" items="${listd}">
	  <tr>
	    <td align="center" >${repairer.repairerID}</td>
	    <td align="center" >${repairer.password}</td>
	    <td align="center" >${repairer.name}</td>
	    <td align="center" >${repairer.sex}</td>
	    <td align="center" >${repairer.phone}</td>
	    <td align="center" >${repairer.remarks}</td>
	    <td align="center">
		    <form action="/dormitory/Sysadmin/sysadmin_modifyrepairer.jsp?repairer.repairerID=${repairer.repairerID}&repairer.password=${repairer.password}		    
		    &repairer.name=${repairer.name}&repairer.sex=${repairer.sex}
		    &repairer.phone=${repairer.phone}&repairer.remarks=${repairer.remarks}" method="post">
			
                <input type ='submit'  value='修改' >
		 	</form>	
		 </td>
		 <td>
		 	<form action="/dormitory/Sysadmin_deleterepairer?repairer.repairerID=${repairer.repairerID}" method="post">
